...
SELECT dea.continent, dea.location, dea.date, dea.population, dea.new_vaccinations, vac.population,
SUM(cast(dea.new_vaccinations as int)) OVER (Partition by dea.location)
FROM PortfolioProject..CovidDeaths as dea
JOIN
PortfolioProject..CovidVaccinations as vac
	ON dea.location = vac.location
...
SELECT dea.continent, dea.location, dea.date, dea.population, dea.new_vaccinations, vac.population,
SUM(CONVERT(int,dea.new_vaccinations)) 
